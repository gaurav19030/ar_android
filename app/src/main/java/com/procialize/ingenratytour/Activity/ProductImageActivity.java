...
import com.procialize.ingenratytour.Adapter.ProductGridViewAdapter;
...
public class ProductImageActivity extends AppCompatActivity {
    ...
    GridView imagegrid;
    ProductGridViewAdapter ImagegGridAdapter;
...
//            actionBar.setHomeAsUpIndicator(R.drawable.Your_Icon); // set a custom icon for the default home button
        ...
        if (Util.isNetworkConnected(this)) {
            GetGallery(Integer.parseInt(id),token);
            try {
                if (pd == null) {
                    pd = new Util.TransparentProgressDialog(ProductImageActivity.this);
                    pd.show();
                }
            }catch (Exception e)
            {
    ...
    public void GetGallery(int id,String token) {
        mAPIService.ProductGalleryGet(new ProductImagepost(id,token)).enqueue(new Callback<ProductImages>() {
            @Override
    ...
    public void showResponse(Response<ProductImages> response) {
        ...
        if (response.body().getSuccess().equals("true")) {
            ...
            ImagegGridAdapter = new ProductGridViewAdapter(this,response.body().getData());
